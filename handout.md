...
- **Deployed to `tideover.site`** (2026-07-10): cross-compiled (`GOOS=linux GOARCH=amd64 CGO_ENABLED=0`), `scp`'d up, old binary kept as `/usr/local/bin/liveurld.bak`, service restarted — `0002_password_auth.sql` applied automatically on startup with no errors. Verified for real against the live domain, not just locally: `POST /api/signup`/`/api/login` over real TLS, the returned token working against `/api/tunnels`, duplicate-email signup rejected with `409`, wrong-password login rejected with `401`, the auth rate limiter's 6th rapid attempt returning `429`, the dashboard HTML serving the new Sign up/Log in/Have a token? tabs, and the control API/Postgres/Redis ports still unreachable from outside — all against `https://tideover.site`.

---

## 7b. Build timeline — going public, account recovery

Two follow-ups from the same session, done together after the user asked "what changes if I make the repo public" and then decided to.

**Made the repo public.** The actual reason private was blocking things: Homebrew/npm/winget/manual-download all resolve to downloading a GitHub Release asset from this repo, and private-repo Release assets need an authenticated, authorized session — so *nobody* outside invited collaborators could install the built binaries at all, independent of whether the Homebrew/npm publishing secrets ever got configured. `go install .../cmd/liveurl@latest` was blocked the same way. Before flipping visibility: pulled the production Elastic IP, EC2 instance ID, and security group ID out of `handout.md` into a new `ops-notes.local.md` (added to `.gitignore` via a `*.local.md` pattern) so the now-public handout doesn't hand out infra reconnaissance. The user then flipped visibility to public manually via GitHub Settings (no `gh` CLI on this machine to automate it).

**Added operator-driven account recovery**, closing the sharpest gap flagged when asked "will users have issues with this": self-serve signup has no email verification and no self-service password reset, and `/api/signup` deliberately refuses to let someone re-signup over an email that's already registered — so a user who forgot their password had *no path back into their account at all* short of the operator hand-editing the database.
- **`Store.ResetPassword(ctx, email, newPassword)`** (`internal/store/users.go`): sets/replaces a password on an existing account by email. Deliberately **not** guarded the way `SignUp` is — it works even on a password-less `liveurld seed`-created account — because this is an operator-only path (run via CLI on the box, never exposed over HTTP), and an operator with shell access already has unrestricted DB access anyway, so the anti-takeover restriction on the public signup endpoint doesn't apply here.
- **`liveurld reset-password --email you@x.com`** (`cmd/liveurld/main.go`): generates a random password, calls `ResetPassword`, mints a fresh token via the existing `NewToken`, and prints both — mirroring `seed`'s output style. Explicitly does **not** verify the requester's identity itself; the doc comment and `--long` help text both say this is the operator's job, out-of-band.
- Dashboard's Log In panel now has a small "forgot your password? contact whoever runs this server" hint instead of a silent dead end on a failed login.
- **Verified for real, not just via tests**: `liveurld seed` to create a password-less account, `liveurld reset-password` on it (printed a password + token), then a live `POST /api/login` against a locally-running `liveurld` with that exact password — got back a working token. Also caught that Docker Desktop (hosting local Postgres/Redis) wasn't running when this work started, which meant the new `ResetPassword` tests were silently *skipping* rather than passing — restarted Docker, reran with `-v`, confirmed all three actually execute against real Postgres.
- **Not built**: real self-service password reset (would need an outbound email provider — a bigger, separate decision involving a new third-party service and possibly DNS records on `tideover.site`, not something to just do silently). This command is the stopgap, not the end state.
- **Not yet deployed to `tideover.site`** — this exists locally/on GitHub only as of the end of this session; production is still running the pre-recovery-command binary. Same deploy cycle as §7a whenever it happens.
...
1. ~~No GitHub remote, no tagged release~~ **Done** — see §8a. Pushed to `Tehman700/liveurl` (now **public**, see §7b) and tagged `v0.1.0` on 2026-07-10; check `https://github.com/Tehman700/liveurl/actions` for that run's actual result.
2. **Homebrew tap / npm publish / winget PR are all scaffolded but not executed** — each needs a one-time manual account/secret setup step from the user (see §7's Distribution section for exactly what each needs). The release workflow now degrades gracefully without them (§8a) rather than failing, so this is purely "not yet done," not "broken." The repo being public (§7b) removes the *other* blocker these all shared — raw binary downloads now actually work.
3. ~~No self-serve signup~~ **Done** — see §7a. `POST /api/signup`/`/api/login` on the dashboard's API now let users create an account and get a token without an operator running `liveurld seed`. ~~No account recovery~~ **Also done** — see §7b, `liveurld reset-password`. Still genuinely open: no *self-service* email-based password reset (needs an outbound email provider, a real setup decision, not done), and no billing/plan tiers.
...
Read this file, then:
- `cd "c:\Users\tehma\Desktop\Live URL Project"`, run `git log --oneline` and `git status` — pushed to `https://github.com/Tehman700/liveurl.git` and tagged `v0.1.0` as of 2026-07-10 (§8a). **Repo is public** (confirmed via an anonymous `curl https://api.github.com/repos/Tehman700/liveurl` returning `"private": false` — check this again before assuming, since visibility can change).
- Confirm production is still healthy: SSH in per `ops-notes.local.md` and run `sudo systemctl status liveurld`, and/or just visit `https://tideover.site/dashboard`. Production is running the **signup-capable** binary (deployed 2026-07-10, §7a) but **not yet** the **account-recovery** binary (`liveurld reset-password`, built 2026-07-10, §7b, still local/GitHub-only as of end of session) — don't assume `reset-password` works on the box without redeploying first. Old binary kept at `/usr/local/bin/liveurld.bak` if a rollback is ever needed.
- If asked to "continue," the natural next steps, per the user's own priorities so far, are the remaining §9 items — most concretely, deploying the §7b account-recovery work to production, and the one-time Homebrew tap / npm token setup (§9 item 2) that's still blocking those two distribution channels from actually publishing (the release workflow itself is done and degrades gracefully without them, §8a; the repo being public already unblocked raw binary downloads and `go install`). Anything that pushes to a real remote, redeploys production, or similar remains a "visible to others / hard to reverse" action this project's own norms say to confirm with the user first (AWS provisioning, DNS changes, production redeploys, and the GitHub push/tag/visibility change were all confirmed with the user before executing this session).
